...
template <class T>
__global__ void clear_table(const unsigned table_size, const T value, T *table)
{
    ...
    unsigned *num_probes_required = NULL)
{
        ...
        if (thread_index >= n_queries)
                return;
        unsigned long long key = keys_in[thread_index];
...
#pragma unroll
        for (int i = kNumHashFunctions - 2; i >= 0; --i) {
                next_location =
                    (previous_location == locations[i] ? locations[i + 1]
                                                       : next_location);
        }
    ...
    unsigned *iterations_taken)
{
        ...
        bool success = insert<kNumHashFunctions>(
            ...
            table, stash_count, entry, &iterations);
